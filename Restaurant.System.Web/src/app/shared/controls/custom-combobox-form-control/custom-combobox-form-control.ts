...
export class RSComboboxFormControl<TData extends Record<string, unknown>, TValue = unknown> extends RSFormControlBase {
  public searchTerm$ = signal('');
  searchTerm = this.searchTerm$.asReadonly;
  setSearchTerm(searchTerm: string) { this.searchTerm$.set(searchTerm) }
  ...
  filteredItems = this.items$.asReadonly();
  ...
  _setOptions() {
    this.setItems(this._getComboboxItems());
  }
  ...
  private _getComboboxItems(): DropDownItem<TValue>[] {
    ...
    const valueField = this.getOptionItem<keyof TData>(RSComboboxFormControlOptions.valueField)!;
    const compositeField = this.getOptionItem<(x: TData) => string>(RSComboboxFormControlOptions.compositeTitle)!;

    let data = this.getDataItems(titleField, valueField, 'To use Combobox, you should specify both "ComboboxTitleField or ComboboxCompositeTitle" and "ComboboxValueField"', RSComboboxFormControlOptions.items, compositeField);
    if (!data) {
      data = [];
    }
